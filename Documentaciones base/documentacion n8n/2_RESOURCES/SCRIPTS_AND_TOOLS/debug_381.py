...
with open('exec_381_raw.json', 'r') as f:
    data = json.load(f)
...
# Extract specific pointers for 381
print("\n--- Raw Response Objects ---")
# Gemini Planner response index might differ, but let's try to find it in the data
for i, x in enumerate(data):
    if isinstance(x, str) and "thought" in x.lower() and "next_instruction" in x.lower():
        print(f"Index {i}: {x}")
