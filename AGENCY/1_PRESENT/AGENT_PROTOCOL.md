...
*   **Read the Manual First:** Before executing any task, you MUST consult `AGENCY/README.md` and the relevant sub-protocols (e.g., `AGENT_PROTOCOL.md`, `JULES_SCRUM_PROTOCOL.md`). Your actions must align with the established architectural patterns.
...
| `frontend` | `Visor Repo` | Astro/React Frontend Components. |
| `backend` | `Supabase` | Supabase Edge Functions & SQL. |
| `documentation` | `AGENCY/` | Protocols, Knowledge Base, and Forensics. |
| `infrastructure` | `Render` | Deployment Configuration (Render). |
| `legacy` | `Legacy Repo` | Old Express backend (reference only). |
